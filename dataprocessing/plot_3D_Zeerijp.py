...
class plot3D:
    ...
    def diff_data(self,filename,axis,step,xyaxis,delta,filename_tri=False,edgecolor=False,
                  friction=0,dyn=0,seis=0,xlim0=False,xlim1=False,zlim0=False,zlim1=False,
                  points=[],hlines=[]):
        """
        ...
        zi = np.linspace(z.max(),z.min(), 4000)
        i = step[0]
        ...
        # diff data
        if axis < 4:
            z3 = (d2_dat_trac_sta[:,axis,i-1])/1e6
            if delta:
                z3 = z3 - (d2_dat_trac_sta[:,axis,1])/1e6

        if xyaxis == 0:
            xi1, zi1 = np.meshgrid(xi, zi)
            si1 = griddata((x, z), z1, (xi1, zi1), method='linear')
        else:
            yi1, zi1 = np.meshgrid(yi, zi)
            si1 = griddata((y, z), z1, (yi1, zi1), method='linear')
        # print(si1.max(),si1.min())

        if xyaxis == 0:
            xi2, zi2 = np.meshgrid(xi, zi)
            si2 = griddata((x2, z2), z3, (xi2, zi2), method='linear')
        else:
            yi2, zi2 = np.meshgrid(yi, zi)
            si2 = griddata((y2, z2), z3, (yi2, zi2), method='linear')

        sid = si1 - si2

        if axis==2 and delta:
            if axis < 4:
                z1 = (self.dat_trac_sta[:,3,i-1])/1e6
                if delta:
                    z1 = z1 - (self.dat_trac_sta[:,3,1])/1e6

            # diff data
            if axis < 4:
                z3 = (d2_dat_trac_sta[:,3,i-1])/1e6
                if delta:
                    z3 = z3 - (d2_dat_trac_sta[:,3,1])/1e6
        ...
        if axis==2 and friction>0:
            z1 = np.sqrt(self.dat_trac_sta[:,0,i-1]**2+self.dat_trac_sta[:,1,i-1]**2)/1e6

            # diff data
            z3 = np.sqrt(d2_dat_trac_sta[:,0,i-1]**2+d2_dat_trac_sta[:,1,i-1]**2)/1e6
            ...
            sid3 = si1 - si2 
            sid  = sid3 - sid*(-1)*friction # negative compressive


        fig = plt.figure(figsize=(10,6))
        ax0 = fig.add_subplot(1, 1, 1)
        if xyaxis == 0:
            pt0 = ax0.imshow(sid,extent=(x.min(),x.max(),z.min(),z.max()))  
        else:
            pt0 = ax0.imshow(sid,extent=(y.min(),y.max(),z.min(),z.max()))  
        cbar = fig.colorbar(pt0)

        if filename_tri:
            z0 = (self.dat_trac_sta[:,3,i-1])/1e6
            z0 = z0 - (self.dat_trac_sta[:,3,1])/1e6
            z0 = np.ma.masked_where(z0 < -0.4, z0)
            flt_tri = np.load(filename_tri)
            flt_tri = flt_tri - 1
            ...
            NbLevels = 256        
            if edgecolor:
                pt0=ax0.tripcolor(triang, z0, NbLevels,cmap='gray', edgecolor="black",alpha=0.2) # cmap=plt.cm.jet,
            else:
                pt0=ax0.tripcolor(triang, z0, NbLevels,cmap='gray',alpha=0.2) # cmap=plt.cm.jet,


        if len(points)>0:
            for i in range(points.shape[0]-1):
                ax0.plot(points[i:i+2,0], points[i:i+2,1], 'c--',alpha=0.95)
        ...
        if len(hlines)>0:
            ...
            hlid = 0
            for hline in (hlines):
                ax0.axhline(y=hline,color=colorhl[str(hlid)],alpha=0.95)
    ...
    def diff_data_func(self,filename,axis,step,xyaxis,delta,filename_tri=False,edgecolor=False,
                  friction=0,dyn=0,seis=0,xlim0=False,xlim1=False,zlim0=False,zlim1=False,
                  points=[],hlines=[]):
        """
        calculate the difference between the results
    ...
    def plot_dyn_slip_tri_group(self,step,axis,vabs=0,
                        mask=0,xyaxis=0,vline=False,zlim0=False,zlim1=False,xlim0=False,xlim1=False,
                        edgecolor=False):
        print ("printing dynamic at " + step)
        fig = plt.figure(figsize=(22,28),tight_layout=True)
        heights = [1, 1, 1, 1, 0.08]
        gs = gridspec.GridSpec(5, 2,height_ratios=heights)
        # frame_list = np.arange(0,100,25)
        frame_list = [0,10,20,40]
        # frame_list = [0,20,40,80]
        ngsax = 0
        for i in frame_list:
            # data 1
