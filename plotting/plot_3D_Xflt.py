...
class plot3D:
    def __init__(self, file,xflt, dyn,seis):
        self.file = file
        self.dyn = int(dyn)
        self.seis = int(seis)
        ...
        if xflt>0:
            self.dat_tracx_sta = self.mdict['tracx_sta']
            self.dat_slipx_sta = self.mdict['slipx_sta']
        if self.dyn>0:
...
# 4. make the dot production, which is the projection. 
    ...
    def plot_Xflt_rup_ct(self,step,dsp,xlim0=False,xlim1=False,zlim0=False,zlim1=False,):
        tol = 1E-6
        dt = 0 # fix data??
        CropY=[0,0]; CropZ=[0,0]; ResY=2000; ResZ=2000; t_plt=16.
        # ang_dip=np.pi/180*66
        fcoord   = self.fcoord
        dt_slip  = self.dt_dyn
        dat_slip = self.dat_slip_sort[step]

        # identify the fault id (here is an example of using the fault normal vector)
        id1=abs(fcoord[:,6]) > 0.01 # main fault (debug: make a better method to determine which fault it is.)
        id2=abs(fcoord[:,6]) < 0.01 # secondary fault
        print (np.sum(id1))
        print (np.sum(id2))
        print (fcoord.shape-np.sum(id1)-np.sum(id2))

        # dat2 = np.linalg.norm(dat_slip2[:,:2,:],axis=1)
        # dat1 = np.linalg.norm(dat_slip1[:,:2,:],axis=1)
        fcoord1 = fcoord[id1,:]
        fcoord2 = fcoord[id2,:]
        dat_slip2 = dat_slip[id2,:,:]
        dat_slip1 = dat_slip[id1,:,:] # slip data based on the main fault and the aux fault

        cnt1 = 1E9*np.ones(shape=dat_slip1[:,0,0].shape) # contour temp data (the time of the rupture nucleation)
        cnt2 = 1E9*np.ones(shape=dat_slip2[:,0,0].shape) # contour temp data (the time of the rupture nucleation)
        len_tot = dat_slip.shape[-1] # the time step domain (len of total time) (potential bug for different)

        # for the main fault
        if dsp==1: dat = np.linalg.norm(dat_slip1[:,:2,:],axis=1) # norm of displacement on both strike and dip direction
        ...
        for i in range(len_tot):
            ...
            if dsp==1:
                ...
                dat_int = np.sum(dat_slip[:,:2,:i+1],axis=2)*dt_slip
                slip = np.linalg.norm(dat_int,axis=1)
            idrup = np.squeeze(np.where(slip>tol)) # all nodes that move exceeding the threashole
            idfrn = np.squeeze(np.where(cnt1<1E9)) # all the nodes that area previously stable
            set_rup = np.setdiff1d(idrup,idfrn) # new rupture front
                        
            cnt1[set_rup] = t
            if i>int(len_tot*0.15) and set_rup.size==0: # if the rupture front stop growing
                t_rup=t
                break
        print (cnt1.min(),cnt1.max())

        # for the aux faulti
        if dsp==1: dat = np.linalg.norm(dat_slip2[:,:2,:],axis=1) # norm of displacement on both strike and dip direction
        print (dat.min(),dat.max())
        for i in range(len_tot):
            t = i*dt_slip+dt
            if dsp==1:
                ...
                slip = np.linalg.norm(dat_int,axis=1)
            idrup = np.squeeze(np.where(slip>tol)) # all nodes that move exceeding the threashole
            idfrn = np.squeeze(np.where(cnt2<1E9)) # all the nodes that area previously stable
            set_rup = np.setdiff1d(idrup,idfrn) # new rupture front
        ...
        # dip=zi/np.sin(ang_dip)
        dip = zi
        dat_grid = griddata((np.squeeze(fcoord1[:,1]), np.squeeze(fcoord1[:,2])), cnt1, (yi, zi), method='linear')
        ...
        # dip=zi/np.sin(ang_dip)
        dip = zi
        dat_grid = griddata((np.squeeze(fcoord2[:,1]), np.squeeze(fcoord2[:,2])), cnt2, (yi, zi), method='linear')
        # fig, ax = plt.subplots()
    ...
    def plot_Xflt_str(self,step,dsp,xlim0=False,xlim1=False,zlim0=False,zlim1=False,):
        tol = 1E-3
        dt = 0 # fix data??
        CropY=[0,0]; CropZ=[0,0]; ResY=2000; ResZ=2000; t_plt=16.
        # ang_dip=np.pi/180*66
        fcoord   = self.fcoord
        dt_slip  = self.dt_dyn
        dat_slip = np.sqrt(self.dat_trac_sta[:,0,step-1]**2+self.dat_trac_sta[:,1,step-1]**2)/np.abs(self.dat_trac_sta[:,2,step-1])
        # dat_slip = self.dat_trac_sta[:,2,step-1]/1e6
        # identify the fault id
        id1=abs(fcoord[:,6]) > 0.01 # main fault (debug: make a better method to determine which fault it is.)
        id2=abs(fcoord[:,6]) < 0.01 # secondary fault
        print (np.sum(id1))
        print (np.sum(id2))
        ...
        yi = np.linspace(ymin, ymax, ResY)
        zi = np.linspace(zmax, zmin, ResZ)
        yi,zi = np.meshgrid(yi,zi)
        # dip=zi/np.sin(ang_dip)
        dip = zi
        dat_grid = griddata((np.squeeze(fcoord1[:,1]), np.squeeze(fcoord1[:,2])), cnt1, (yi, zi), method='linear')

        pt0 = ax.imshow(dat_grid,extent=(ymin,ymax,zmin,zmax))
        cbar = fig.colorbar(pt0,fraction=0.046, pad=0.04)
        # ax.clabel(cs1, inline=1, fmt = '%1.1f',fontsize=10)
        ...
        yi = np.linspace(xmin, xmax, ResX) # 90 degree intersection
        zi = np.linspace(zmax, zmin, ResZ)
        yi,zi = np.meshgrid(yi,zi)
        # dip=zi/np.sin(ang_dip)
        dip = zi
        dat_grid = griddata((np.squeeze(fcoord2[:,0]), np.squeeze(fcoord2[:,2])), cnt2, (yi, zi), method='linear')
        # fig, ax = plt.subplots()

        pt0 = ax.imshow(dat_grid,extent=(xmin,xmax,zmin,zmax))
        cbar = fig.colorbar(pt0,fraction=0.046, pad=0.04)
        # ax.clabel(cs1, inline=1, fmt = '%1.1f',fontsize=10)
        if xlim0:        
            ax.set_ylim(xlim0,xlim1)
    ...
    # plot dynamic result, fault node coordinate hard coded. using interpretation instead of tri
    def plot_Xflt_str_dyn(self,step,dsp,timedyn,axis=0,xlim0=False,xlim1=False,zlim0=False,zlim1=False):
        tol = 1E-3
        dt = 0 # fix data??
        CropY=[0,0]; CropZ=[0,0]; ResY=1000; ResZ=1000; t_plt=16.
        # ang_dip=np.pi/180*66
        ...
        dt_slip  = self.dt_dyn
        # dat_slip = np.sqrt(self.dat_trac_sta[:,0,step-1]**2+self.dat_trac_sta[:,1,step-1]**2)/np.abs(self.dat_trac_sta[:,2,step-1])
        # dat_slip = np.sqrt(self.dat_trac_sort[step][:,0,timedyn]**2 + self.dat_trac_sort[step][:,1,timedyn]**2)/np.abs(self.dat_trac_sort[step][:,2,timedyn])
        if axis == 0: # SCU (tau/sigma)
            dat_slip = np.sqrt(self.dat_trac_sort[step][:,3,timedyn]**2 + self.dat_trac_sort[step][:,4,timedyn]**2)/np.abs(self.dat_trac_sort[step][:,5,timedyn])
        elif axis == 1: # slip
            dat_slip = np.sqrt(self.dat_slip_sort[step][:,0,timedyn]**2 + self.dat_slip_sort[step][:,1,timedyn]**2)
        # dat_slip = np.abs(self.dat_trac_sort[step][:,2,timedyn]/1e6)
        # dat_slip = np.sqrt(self.dat_slip_sort[step][:,0,timedyn]**2 + self.dat_slip_sort[step][:,1,timedyn]**2)
        # identify the fault id
        id1=abs(fcoord[:,6]) > 0.01 # main fault (debug: make a better method to determine which fault it is.)
        id2=abs(fcoord[:,6]) < 0.01 # secondary fault
