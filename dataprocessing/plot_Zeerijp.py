    ...
    def plot_dyn_slip_tri_group(self,tri_file_1,frame_list,step,axis,vabs=0,
        ...
        for i in frame_list:
            ...
            if edgecolor:
                ...
                pt0=ax0.tripcolor(triang, z1, NbLevels,vmin = vmin/10, vmax = vmax/10)
            if zlim0 or zlim1:
                ax0.set_ylim(zlim0,zlim1)
            if xlim0 or xlim1:
                ax0.set_xlim(xlim0,xlim1)
...
# 4. make the dot production, which is the projection. 
    ...
    def plot_induced_static_tri_group(self,filename,tstep_list,delta=False,
        ...
        for i in range(4):
            ...
            if interp and i==3:
                ...
                levels = np.array([0.999,1]) # levels = np.array([0,0.2,0.3,0.9,1.0,1.1])
                ax1.tricontour(triang, z1,levels=levels, colors='black')
                kk_list = []
                z1_triface = np.copy(z1)
                for kk in range(len(z1_triface)):
                    if z1_triface[kk]<0:
                       z1_triface[kk]=1
                pt0=ax1.tripcolor(triang, z1_triface, shading='flat',cmap='Reds')

                levels = np.array([-1,0]) # levels = np.array([0,0.2,0.3,0.9,1.0,1.1])
                ax1.tricontour(triang, z1,levels=levels, colors='w')
                #ax1.xaxis.set_major_locator(plt.MaxNLocator(3))
                n_levels_cf = np.array([-1,0])
                # n_levels = 4
                tcf = ax1.tricontourf(triang, z1, n_levels_cf,
                               colors="none",
                               hatches=["\\\\", None,"///", "\\", None, "\\\\", "*"])
    ...
    def plot_induced_static_tri_group_func(self,filename,tstep_list,axis,delta=False,
        ...
        # SCU 
            ...
            dc = 0.01
            mu_node = 0.55*np.ones(len(self.dat_slip_sta[:,1,i-1])) # friction coefficient 
            ...
            for j in range(len(mu_node)):
                if slip_node[j]>dc:
                    ...
                    mu_node[j] = 0.55 - 0.15*(slip_node[j]/dc)
    ...
    def diff_data_triint(self,filename,step_,step_cri,xyaxis,delta,filename_tri0,filename_tri1,
        ...
        del d2_mdict
        fig = plt.figure(figsize=(20,14),tight_layout=True,dpi=300)
        gs = gridspec.GridSpec(2, 2)
        ...
        for i in range(4):
            ...
            if xyaxis == 1:
                ...
                if axis == 3:
                    # pt0 = ax1.imshow(zi_lin0,extent=(intx_lim0,intx_lim1,intz_lim1,intz_lim0),aspect='auto',alpha=1)
                    pt0 = ax_list[i].tripcolor(triang00, z100, shading='flat',cmap='Reds' ) # cmap=plt.cm.jet,
                    # contour for scu = 1
                    # levels = np.array([0.99,1.0]) # levels = np.array([0,0.2,0.3,0.9,1.0,1.1])
                    # ax1.tricontour(triang, z1,levels=levels, colors='k')

                    n_levels = np.array([-1,-0.99999,0.99999,1])
                    # n_levels = 4
                    tcf=ax_list[i].contourf(zi_lin, n_levels,
                                       colors="none",
                                       extent=(intx_lim0,intx_lim1,intz_lim0,intz_lim1),
                                       hatches=["\\\\", None,"///", "\\", None, "\\\\", "*"])
