...
import os, sys
import meshio
sys.path.append('/palmyra/data/jingmingruan/')
...
name_out = "Buijze3D_xflt.inp"
save_checkpoint = 0
load_checkpoint = 1
conv_Xflt = 0 # turn this off if xflt
nmp = 40

init_sigmay = 0
load_vtk = 0

load_init_BC = 1 # turn this on if load_init_Xflt
load_init_Xflt = 1
...
bod_frc=1; hyb=1; nviz_dyn=500; nviz_wave=500; nviz_slip=500
alpha= 0.; beta = 0.00125; rfrac = 0

init=0; init_BC=0; reform_BC=0
if load_init_BC or load_init_Xflt: init_BC=1
...
for i in [37,38]: # input fault boundary node & intersection # debug: one fault
...
if Xflt>1 or conv_Xflt:
    ...
    # debug add the fault vector predefined here=====
    ft_pos_index_main = np.copy(ft_pos_dir_main) # positive direction for the aux fault x y z 
    ft_pos_index_aux = np.copy(ft_pos_dir_aux)   # positive direction for the aux fault x y z 
    ft_pos_index_main[:,:] = 0
    ft_pos_index_aux[:,:] = 1
    ft_pos_index = np.vstack((ft_pos_index_main, ft_pos_index_aux))
    # ================================================
else:
    ft_pos_dir = ft_pos_dir_main
...
# %%
# print (ft_pos_index.shape)
# print (indices.shape)
# print (ft_pos_nodes.shape)
# print (vec_fn.shape)
# print (ft_xinit_nodes.shape)
...
if Xflt>1:
    nxfnd = len(ft_xinit_nodes) * 2
else:
    nxfnd = len(ft_xinit_nodes)
...
ft_pos_index = ft_pos_index.flatten()[indices]
ft_pos_index = ft_pos_index[np.in1d(flt_node,flt_xbdr_node,invert=True)]
for i in range(len(ft_pos_index)):
    if ft_pos_index[i] == 0:
        vec_fs[i,:] = pre_vec_fs_main
        vec_fd[i,:] = pre_vec_fd_main
        vec_fn[i,:] = pre_vec_fn_main
    else:
        vec_fs[i,:] = pre_vec_fs_aux
        vec_fd[i,:] = pre_vec_fd_aux
        vec_fn[i,:] = pre_vec_fn_aux
        
vec_fs[len(ft_pos_nodes):,:] = pre_vec_fs_main
vec_fd[len(ft_pos_nodes):,:] = pre_vec_fd_main
vec_fn[len(ft_pos_nodes):,:] = pre_vec_fn_main
# ================================================
...
f2_ax1.scatter(zcnt,vec_fs[:138085,0],  alpha=0.6,cmap='jet') # debug



f2_ax2 = fig.add_subplot(spec2[1])
f2_ax2.scatter(zcnt,vec_fd[:138085,2], alpha=0.6,cmap='jet')
...
if save_checkpoint:
    ...
    fout_h5 = 'Zeerijp3D_checkpoint_Xflt.h5'
    try:
        os.remove(fout_h5)
    except OSError:
        pass
...
if load_checkpoint:
    ...
    f_checkpoint = h5py.File('Zeerijp3D_checkpoint_Xflt.h5', 'r') # debug:chang
...
# %%
from plot_3D import *
if load_init_BC or load_init_Xflt:
...
# %%
print (st_init[len(ft_pos_nodes):,:].shape)
print (ft_x_nodes.shape)
...
f2_ax2 = fig.add_subplot(spec2[1])
f2_ax2.scatter(trac_bc3[:,2],zcnt2,  alpha=0.6,cmap='jet')
